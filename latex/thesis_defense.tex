...
\begin{frame}{Radar model derivation assumptions}
 \begin{itemize}
  \item Continuous model holds
  \item No signal is received from outside the sampled delay window
  \item Transmitted waveform is piecewise-constant in equal length bauds
 \end{itemize}
\end{frame}
...
\begin{frame}{Waveform inversion equation}
 Rewrite matched filter using the model in terms of solution $h$:
 \begin{align*}
  A^*(y) &= A^*\left(A(h) + y - A(h)\right)\\
  &= A^*A(h) + A^*\left(y - A(h)\right)\\
  &= \left(\frac{1}{N}I + A^*A - \frac{1}{N}I\right)(h) + A^*\left(y - A(h)\right)\\
  &= \frac{1}{N}h + \left(A^*A - \frac{1}{N}I\right)(h) + A^*\left(y - A(h)\right)
 \end{align*}
 Ambiguity function is $A^*A$, and sidelobes are $A^*A - \frac{1}{N}I$.
 
 The matched filter is usually scaled by a factor of $\sqrt{N}$, so the waveform inversion result with sidelobes removed is given by:
 \begin{equation*}
