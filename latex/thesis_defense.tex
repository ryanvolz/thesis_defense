...
\begin{frame}<1-5>[label=contributions]{Contributions}
 ...
 \begin{enumerate}
  \item<2,7-> Developed a discrete \alert{radar model} that captures signal sparsity and discovered its connections to existing techniques.
  \item<3,8-> Formulated the model representation of \alert{off-grid and distributed scatterers} and analyzed it for accuracy and preservation of sparsity.
  \item<4,9-> Created and tuned an \alert{efficient implementation} of model inversion using modern convex optimization techniques.
  \item<5,10-> Demonstrated the effectiveness of the inversion technique for the \alert{removal of filtering artifacts} from real ionospheric radar data.
 \end{enumerate}
\end{frame}
