...
\begin{frame}{The world is sparse}
 \begin{definition}
  \vspace{-0.5ex}
  \begin{itemize}
   \item A vector is \alert{sparse} if only a small portion of its elements is nonzero.
   \item Additionally, the term is often applied loosely to when this is approximately true (i.e. most of the elements are close to zero).
  \end{itemize}
  \vspace{-0.5ex}
 \end{definition}
 \begin{columns}
...
%  \centering
%  \includegraphics[width=0.7\textheight]{ionosphere_communications}
%  \begin{itemize}
...
\begin{frame}{Ionospheric environment: meteoroids and meteors}
 \begin{columns}
  ...
  \end{definition}
  \raggedleft
  \includegraphics[width=\textwidth]{space_bodies}\\
  {\tiny Ceplecha (1998), Meteor Phenomena and Bodies}
  \column{0.5\textwidth}
  \begin{definition}[Meteor]
   Plasma created by meteoroid entering the atmosphere.
  \end{definition}
  \raggedleft
  \includegraphics[width=\textwidth]{leonid_meteor_2009}\\
  {\tiny CC BY 2.0, http://www.flickr.com/photos/edsweeney/}
 \end{columns}
\end{frame}
...
\begin{frame}{Measuring the ionosphere}
 ...
 \vspace{2ex}
 Range of ionospheric regions: \onslide<2>{mid-latitude,} \onslide<3>{equatorial,} \onslide<4>{polar.}
...
\begin{frame}{Measurement challenges}
 \begin{columns}
  ...
  \begin{enumerate}
   ...
   \item \alert{Self-interference} from range-spread targets\\
   (e.g. meteor trails)
...
\begin{frame}{Sparsity and radar}
...
\begin{frame}<1-5>[label=contributions]{Contributions}
 In order to enable flexible high-resolution measurements of ionospheric plasma phenomena, including interfering and range-spread targets, I made the following contributions:
 \begin{enumerate}
  \item<2,7-> Developed a discrete radar model suitable for signal reconstruction using sparsity-based inversion techniques.
  \item<3,8-> Formulated the model representation of off-grid and distributed scatterers and analyzed it for accuracy and preservation of sparsity.
  \item<4,9-> Created and tuned an efficient implementation of model inversion using modern convex optimization techniques.
  \item<5,10-> Demonstrated the effectiveness of the inversion technique for the removal of filtering artifacts from ionospheric radar data.
 \end{enumerate}
\end{frame}
...
\begin{frame}{The matched filter}
 ...
 \end{example}
 \centering
 \movie[autostart,loop,poster,width=4in,height=1.501730in]{}{autocorrelation_animation_b13_msl.mp4}
\end{frame}
...
\begin{frame}{Effect of sidelobes}
 Sidelobes are the primary reason that outstanding ionospheric science questions cannot be satisfactorily answered.
 \vspace{1ex}
 \begin{columns}
  \column{0.25\textwidth}
  \begin{beamercolorbox}{example text}
...
\begin{frame}{Sidelobe mitigation}
 \begin{block}{Delay-frequency sidelobe mitigation in use}
  \begin{itemize}
   ...
   \item Range sidelobe inverse filters
  \end{itemize}
 \end{block}
...
\begin{frame}{Inverse filter ambiguity function}
 \begin{definition}
  The \alert{inverse filter} is designed to produce no delay sidelobes (for loss of SNR) if filtering is done at the target's frequency.
 \end{definition}
 \begin{columns}
  \column{0.5\textwidth}
  \centering
...
\begin{frame}{Model needed}
 Need to formally conceptualize radar problem in order to employ sparsity and compressed sensing techniques.
\end{frame}
...
\begin{frame}{Radar model from matched filter}
 \begin{columns}
  ...
  \begin{itemize}
   ...
   \item Radar model is simply adjoint of matched filter\\(\alert{remember for later})
...
\begin{frame}{Is a discrete model useful?}
 How well does this discrete model represent real (plasma) targets?
\end{frame}
...
\begin{frame}{Model discretization from radar signal equation}
 ...
 \begin{block}{Discretized radar model}
  \vspace{-2ex}
  \begin{equation*}
   y[m] = \sum_{p=0}^{P-1} \sum_{n=0}^{N-1} \frac{1}{\sqrt{N}} s[m - p + L - 1] e^{2\pi i n m / N} h[n,p] = A(h)
  \end{equation*}
  \vspace{-2ex}
 \end{block}
\end{frame}
...
\begin{frame}{First-order step}{Non-smooth}
 For a non-differentiable function, an attractive alternative is the prox step.
 \begin{equation*}
...
\againframe<6->{contributions}

\begin{frame}{Benefits of waveform inversion}
 \begin{columns}
  \column{0.5\textwidth}
  \begin{enumerate}
   \item \alert{Removes sidelobes} -\\
   Differentiates in delay and frequency for use in a crowded environment and for range-spread targets
   \item \alert{High resolution} -\\
   Enables observation of small-scale processes\\
   \item \alert{Flexible} -\\
   Can use many different waveforms
  \end{enumerate}
 \column{0.5\textwidth}
 \includegraphics[width=\textwidth]{equatorial_example_mf_rti_3}\\
 \includegraphics[width=\textwidth]{head_and_flare_mf_rti_3}\\
...
\begin{frame}{Further research}
 \begin{block}{Near-universal radar measurements}
  Submitted NSF postdoc proposal to create near-universal measurement mode across radar chain using uniquely-tuned waveforms.
 \end{block}
 ...
 \begin{block}{Passive radar}
  Waveform flexibility invites use in passive radar where the transmitted waveform cannot be controlled.
 \end{block}
\end{frame}
