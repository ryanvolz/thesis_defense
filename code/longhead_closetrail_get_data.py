...
datadir = '/storage/data/jicamarca/2011_meteor/data/d2011061_00'
...
vlt_o = el.jicamarca.voltage_reader(files[535])

rall = vlt_o.r
rslc = el.slice_by_value(rall, 80000, 120000)
r = rall[rslc]
vlt1 = vlt_o.read_from_block(1, 400, 0, sample_idx=rslc, chan_idx=0)
vlt2 = vlt_o.read_from_block(2, 0, 400, sample_idx=rslc, chan_idx=0)
vlt = np.concatenate((vlt1, vlt2), axis=0)
t = vlt_o.block_times[1] + (400 + np.arange(vlt.shape[0]))*vlt_o.ipp
ts = vlt_o.ts
ipp = vlt_o.ipp
f0 = 49920000.0

noise_vlt = vlt_o.read_from_block(1, 0, 0, sample_idx=el.slice_by_value(rall, None, 80000), 
                                  chan_idx=0)
...
with open('longhead_closetrail.pkl', 'wb') as f:
