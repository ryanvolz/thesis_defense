...
vlt_o = el.jicamarca.voltage_reader(files[585])

rall = vlt_o.r
rslc = el.slice_by_value(rall, 80000, 130000)
r = rall[rslc]
vlt = vlt_o.read_from_block(2, 200, 700, sample_idx=rslc, chan_idx=0)
t = vlt_o.block_times[2] + (200 + np.arange(vlt.shape[0]))*vlt_o.ipp
ts = vlt_o.ts
ipp = vlt_o.ipp
f0 = 49920000.0

noise_vlt = vlt_o.read_from_block(2, 0, 0, sample_idx=el.slice_by_value(rall, None, 80000), 
                                  chan_idx=0)
...
with open('ejet_two_heads_trail.pkl', 'wb') as f:
