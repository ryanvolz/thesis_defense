...
axes[0].text(ts[0] - 4, 0, 'Received', va='center', ha='right')
...
rcvtxt = axes[1].text(ts[0] - 4, 0, 'Transmitted', va='center', ha='right', zorder=-10)
...
sumtxt = axes[2].text(ts[0] - 4, -1, r'$\sum$', va='bottom', ha='right')
sumtxt.set_visible(False)
restxt = axes[2].text(ts[-1] + 4, -0.5, '', va='bottom', ha='left')
...
axes[3].text(ta[0] - 4, 0, 'Filtered', va='bottom', ha='right')
axes[3].set_xlim(-L + 0.5 - L/2.0, L - 0.5 + L/2.0)
#axes[3].set_yticks([0, L])
axes[3].set_ylim(-3.5, 1.25*L)
...
def animate(k):
    ...
    if k < 0:
        ...
        acorrtxt.set_position((ta[k], acorr[k] + 4))
...
anim = animation.FuncAnimation(fig, animate, init_func=init_frame,
                               frames=ks, interval=200, repeat_delay=500, 
                               blit=False)
